...
class DFNameEditor:
    ...
    def __init__(self, root):
        ...
        self.response_cache: Dict[str, str] = {}  # Cache for API responses
        self.progress_file = "df_name_editor_progress.json"  # Progress checkpoint file
        
        # Thread cancellation flags
        self.cancel_generation = threading.Event()  # Flag to cancel generation
    ...
    def on_closing(self):
        """Handle window close event - cancel generation and exit"""
        self.log("Window closing - cancelling generation...")
        self.cancel_generation.set()
        # Wait a bit for threads to finish, then destroy
        self.root.after(1000, self.root.destroy)
    
    def detect_base_path(self):

        """Detect the base path for Dwarf Fortress 0.47.05 only"""

        # Only support 0.47.05 structure
        ...
        # Default to 0.47.05 structure (even if doesn't exist yet)

        return Path("Dwarf Fortress 0.47.05/raw")

    

    def is_version_47(self):

        """Always return True - we only support 0.47.05"""
    ...
    def load_files(self):
        ...
        # Get file paths for 0.47.05 structure: raw/objects/
        ...
        language_files = [
            ...
            "objects/language_SYM.txt",  # Language symbol file
        ...
        for file_path in language_files:
            ...
            if full_path.exists():
                ...
                # language_SYM.txt uses a different parser
                if "language_SYM" in file_path:
                    self.parse_language_sym_file(full_path)
                else:
        ...
        # Descriptors (colors, shapes, patterns)
        self.log("Loading descriptor files...")
        
        descriptor_files = [
            "objects/descriptor_color_standard.txt",
            "objects/descriptor_shape_standard.txt",
            "objects/descriptor_pattern_iris_eye.txt",
            "objects/descriptor_pattern_pupil_eye.txt",
            "objects/descriptor_pattern_special.txt",
        ]
        
        loaded_descriptor_count = 0
        for file_path in descriptor_files:
            full_path = (self.base_path / file_path).resolve()
            if full_path.exists():
                self.parse_descriptor_file(full_path)
                loaded_descriptor_count += 1
            else:
                self.log(f"  ✗ File not found: {full_path}")
        
        self.log(f"Loaded {loaded_descriptor_count} descriptor files")

        # Archivos de texto/frases

        self.log("Loading text files...")

        # Text files - Version 0.47.05: text files are in objects/text/
        ...
        # Note: language_SYM.txt is now loaded with other language files above
        
        # Load material template file
        self.log("Loading material template file...")
        material_template_file = (self.base_path / "objects/material_template_default.txt").resolve()
        if material_template_file.exists():
            self.parse_material_template_file(material_template_file)
            self.log(f"Loaded material template file")
        else:
            self.log(f"  ✗ File not found: {material_template_file}")

        

        # Search subfolders recursively for any additional .txt files
        ...
        if objects_path.exists():
                ...
                for txt_file in objects_path.rglob("*.txt"):
                    ...
                    # Check descriptor files
                    for desc_file in descriptor_files:

                        if (self.base_path / desc_file) == txt_file:
                    ...
                    # Skip language_SYM.txt (already handled in language_files)
                    if txt_file.name == "language_SYM.txt":
                        continue
                    
                    # Skip material_template_default.txt (already handled)
                    if txt_file.name == "material_template_default.txt":
                        continue
                    ...
                    elif "descriptor" in file_name_lower:
                        self.log(f"Found descriptor file: {relative_path}")
                        self.parse_descriptor_file(txt_file_abs)
                    elif "material_template" in file_name_lower:
                        self.log(f"Found material template file: {relative_path}")
                        self.parse_material_template_file(txt_file_abs)
                    elif "body" in file_name_lower or "tissue_template" in file_name_lower or "c_variation" in file_name_lower or "b_detail_plan" in file_name_lower:
        ...
        self.log(f"  Name entries loaded: {len(self.name_entries):,}")
        self.log(f"  Text entries loaded: {len(self.text_entries):,}")
        self.log(f"  Total entries: {len(self.name_entries) + len(self.text_entries):,}")
        self.log(f"  Files checked:")
        self.log(f"    - Creature files: {loaded_creature_count}")
        self.log(f"    - Item files: {loaded_item_count}")
        self.log(f"    - Material files: {loaded_material_count}")
        self.log(f"    - Plant files: {loaded_plant_count}")
        self.log(f"    - Language files: {loaded_language_count}")
        self.log(f"    - Descriptor files: {loaded_descriptor_count}")
        self.log(f"    - Speech files: {loaded_speech_count}")
        self.log(f"    - Interaction files: {loaded_interaction_count}")
        self.log(f"    - Reaction files: {loaded_reaction_count}")
        self.log(f"    - Example files: {loaded_example_count}")
        self.log(f"    - Building files: {loaded_building_count}")
        self.log(f"    - Text files: {loaded_text_count}")
        self.log("=" * 60)
    ...
    def parse_entity_file(self, file_path: Path):
        ...
        """Parsea archivos de entidades - solo tags que se muestran al jugador"""
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        for i, line in enumerate(lines, 1):
            ...
            entity_match = re.match(r'\[ENTITY:(\w+)\]', line)
            if entity_match:
            ...
            if name_match:
                ...
                if len(parts) >= 1:  # Aceptar incluso nombres de una parte
                    context = f"Entity: {current_entity}" if current_entity else "Entity"
                    if current_position:
                        context += f" | Position: {current_position}"
                    entry = NameEntry(str(file_path.resolve()), i, "NAME", line.strip(), parts, context)
                    entry.entry_id = len(self.name_entries)
            ...
            # [NAME_MALE:] y [NAME_FEMALE:] - Nombres de posiciones por género (se muestran)
            name_male_match = re.match(r'\s*\[NAME_MALE:([^\]]+)\]', line)
            if name_male_match:
                parts_str = name_male_match.group(1)
                parts = [p.strip() for p in parts_str.split(':')]
                if len(parts) >= 1:
                    context = f"Entity: {current_entity} | Position: {current_position} (Male)" if current_entity else f"Position: {current_position} (Male)"
                    entry = NameEntry(str(file_path.resolve()), i, "NAME_MALE", line.strip(), parts, context)
                    entry.entry_id = len(self.name_entries)
                    self.name_entries.append(entry)
                    self.entry_dict[entry.entry_id] = entry
                    entries_found += 1
            
            name_female_match = re.match(r'\s*\[NAME_FEMALE:([^\]]+)\]', line)
            if name_female_match:
                parts_str = name_female_match.group(1)
                parts = [p.strip() for p in parts_str.split(':')]
                if len(parts) >= 1:
                    context = f"Entity: {current_entity} | Position: {current_position} (Female)" if current_entity else f"Position: {current_position} (Female)"
                    entry = NameEntry(str(file_path.resolve()), i, "NAME_FEMALE", line.strip(), parts, context)
                    entry.entry_id = len(self.name_entries)
                    self.name_entries.append(entry)
                    self.entry_dict[entry.entry_id] = entry
                    entries_found += 1
            
            # [SQUAD:] - Nombres de escuadrones (se muestran)
            ...
            if squad_match:
                ...
                if len(parts) >= 2:
                    ...
                    context = f"Entity: {current_entity} | Squad ({number} members)" if current_entity else f"Squad ({number} members)"
                    entry = NameEntry(str(file_path.resolve()), i, "SQUAD", line.strip(), 
            ...
            # [NAME_WAR:], [NAME_BATTLE:], [NAME_SIEGE:], etc. - Nombres de símbolos (se muestran en Legends)
            # Estos aparecen después de [SELECT_SYMBOL:...]
            symbol_name_match = re.match(r'\s*\[(NAME_WAR|NAME_BATTLE|NAME_SIEGE|NAME_ROAD|NAME_TUNNEL|NAME_BRIDGE|NAME_WALL|NAME_BUILDING_TEMPLE|NAME_BUILDING_LIBRARY|NAME_ENTITY_MERCHANT_COMPANY):([^\]]+)\]', line)
            if symbol_name_match:
                symbol_type = symbol_name_match.group(1)
                parts_str = symbol_name_match.group(2)
                parts = [p.strip() for p in parts_str.split(':')]
                if len(parts) >= 1:
                    context = f"Entity: {current_entity} | Symbol: {symbol_type}" if current_entity else f"Symbol: {symbol_type}"
                    entry = NameEntry(str(file_path.resolve()), i, symbol_type, line.strip(), parts, context)
                    entry.entry_id = len(self.name_entries)
                    self.name_entries.append(entry)
                    self.entry_dict[entry.entry_id] = entry
                    entries_found += 1
        ...
        if entries_found == 0:
            self.log(f"    ⚠ No entity name entries found in {file_path.name}")
        else:
            self.log(f"    ✓ Found {entries_found} entity name entries in {file_path.name}")
    ...
    def parse_language_file(self, file_path: Path):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if tword_match:
                ...
                english = tword_match.group(1).strip()
                translation = tword_match.group(2).strip()
                context = f"Translation: {current_word}" if current_word else "Translation"
                entry = NameEntry(str(file_path), i, "T_WORD", line.strip(), [english, translation], context)
                entry.entry_id = len(self.name_entries)
            ...
            # [CDI:VERB:singular:plural:adjective] - Verbos de interacción (en language_words.txt)
            cdi_verb_match = re.match(r'\s*\[CDI:VERB:([^\]]+)\]', line)
            if cdi_verb_match:
                parts_str = cdi_verb_match.group(1)
                parts = [p.strip() for p in parts_str.split(':')]
                if len(parts) >= 2:
                    context = f"Word: {current_word} (CDI)" if current_word else "Language CDI Verb"
                    entry = NameEntry(str(file_path.resolve()), i, "CDI_VERB", line.strip(), parts, context)
                    entry.entry_id = len(self.name_entries)
                    self.name_entries.append(entry)
                    self.entry_dict[entry.entry_id] = entry
                    cdi_verb_count += 1
        ...
        entries_added = entries_after - entries_before
        if entries_added > 0:
            self.log(f"    ✓ {file_path.name}: Added {entries_added} entries (NOUN: {noun_count}, ADJ: {adj_count}, VERB: {verb_count}, T_WORD: {tword_count}, CDI_VERB: {cdi_verb_count})")
        else:
            self.log(f"    ⚠ {file_path.name}: No entries found")
    ...
    def parse_descriptor_file(self, file_path: Path):
        """Parsea archivos de descriptors: [COLOR:...] y [SHAPE:...] con [NAME:...] dentro"""
        if not file_path.exists():
            self.log(f"    ✗ Descriptor file does not exist: {file_path}")
            return
        ...
        try:
            ...
            self.log(f"    Reading descriptor file {file_path.name} ({len(lines)} lines)...")
            entries_found = 0
            current_color = ""
            current_shape = ""
            
            for i, line in enumerate(lines, 1):
                # Track current COLOR or SHAPE
                color_match = re.match(r'\[COLOR:(\w+)\]', line)
                if color_match:
                    current_color = color_match.group(1)
                    continue
                
                shape_match = re.match(r'\[SHAPE:(\w+)\]', line)
                if shape_match:
                    current_shape = shape_match.group(1)
                    continue
                
                # [NAME:...] dentro de COLOR o SHAPE
                name_match = re.match(r'\s*\[NAME:([^\]]+)\]', line)
                if name_match:
                    parts_str = name_match.group(1)
                    parts = [p.strip() for p in parts_str.split(':')]
                    if len(parts) >= 1:
                        if current_color:
                            context = f"Color: {current_color}"
                            tag_type = "COLOR_NAME"
                        elif current_shape:
                            context = f"Shape: {current_shape}"
                            tag_type = "SHAPE_NAME"
                        else:
                            context = f"Descriptor: {file_path.name}"
                            tag_type = "DESCRIPTOR_NAME"
            ...
            if entries_found == 0:
                self.log(f"    ⚠ No descriptor name entries found in {file_path.name}")
            else:
                self.log(f"    ✓ Found {entries_found} descriptor name entries in {file_path.name}")
        except Exception as e:
            import traceback
            self.log(f"    ✗ Error parsing descriptor file {file_path.name}: {e}")
            self.log(f"    {traceback.format_exc()}")
    
    def parse_material_template_file(self, file_path: Path):
        """Parsea archivos de material_template: [STATE_NAME:...] dentro de [MATERIAL_TEMPLATE:...]"""
        if not file_path.exists():
            self.log(f"    ✗ Material template file does not exist: {file_path}")
            return
        ...
        try:
            ...
            self.log(f"    Reading material template file {file_path.name} ({len(lines)} lines)...")
            entries_found = 0
            current_template = ""
            
            for i, line in enumerate(lines, 1):
                # Track current MATERIAL_TEMPLATE
                template_match = re.match(r'\[MATERIAL_TEMPLATE:(\w+)\]', line)
                if template_match:
                    current_template = template_match.group(1)
                    continue
                
                # [STATE_NAME:state:name] - Names for material states (solid, liquid, gas)
                state_name_match = re.match(r'\s*\[STATE_NAME:([^:]+):([^\]]+)\]', line)
                if state_name_match:
                    state = state_name_match.group(1).strip()
                    name = state_name_match.group(2).strip()
                    context = f"Material Template: {current_template}, State: {state}"
                    entry = NameEntry(str(file_path.resolve()), i, "STATE_NAME", line.strip(), [name], context)
                    entry.entry_id = len(self.name_entries)
                    self.name_entries.append(entry)
                    self.entry_dict[entry.entry_id] = entry
                    entries_found += 1
            ...
            if entries_found == 0:
                self.log(f"    ⚠ No state name entries found in {file_path.name}")
            else:
                self.log(f"    ✓ Found {entries_found} state name entries in {file_path.name}")
        except Exception as e:
            import traceback
            self.log(f"    ✗ Error parsing material template file {file_path.name}: {e}")
            self.log(f"    {traceback.format_exc()}")
    ...
    def save_file(self, file_path: str, entries: List[NameEntry]):
        ...
        for line_num, entry in changes.items():
            ...
            if line_num < len(lines):
                ...
                if entry.tag_type == "SQUAD" and len(entry.parts) >= 3:
                ...
                elif entry.tag_type in ["NAME", "CASTE_NAME", "NAME_MALE", "NAME_FEMALE"]:
                    # NAME, CASTE_NAME, NAME_MALE, NAME_FEMALE: singular:plural:adjective (3 partes máximo)
                    if len(entry.parts) >= 3:
                ...
                elif entry.tag_type.startswith("NAME_") and entry.tag_type not in ["NAME_MALE", "NAME_FEMALE"]:
                    # NAME_WAR, NAME_BATTLE, NAME_SIEGE, etc. - Nombres de símbolos (1-3 partes)
                    if len(entry.parts) >= 3:
                    ...
                    if len(entry.parts) >= 2:
                        ...
                        new_line = f"{indent}[T_WORD:{entry.parts[0]}:{entry.parts[1]}]\n"
                    else:

                        new_line = f"{indent}[T_WORD:{entry.parts[0]}]\n"
                elif entry.tag_type == "CDI_VERB":
                ...
                elif entry.tag_type in ["COLOR_NAME", "SHAPE_NAME", "DESCRIPTOR_NAME"]:
                    # Descriptors: [NAME:singular] o [NAME:singular:plural]
                    if len(entry.parts) >= 2:
                        new_line = f"{indent}[NAME:{entry.parts[0]}:{entry.parts[1]}]\n"
                    else:
                        new_line = f"{indent}[NAME:{entry.parts[0]}]\n"
                elif entry.tag_type == "STATE_NAME":
                    # STATE_NAME:state:name (material templates)
                    # Need to find the state from context or keep original
                    # For now, use first part as name
                    if len(entry.parts) >= 1:
                        # Try to extract state from original line or context
                        original_line = entry.original_line
                        state_match = re.search(r'\[STATE_NAME:([^:]+):', original_line)
                        if state_match:
                            state = state_match.group(1)
                            new_line = f"{indent}[STATE_NAME:{state}:{entry.parts[0]}]\n"
                        else:
                            # Fallback: just use the name
                            new_line = f"{indent}[STATE_NAME:ALL_SOLID:{entry.parts[0]}]\n"
                    else:
                        new_line = entry.original_line + "\n"
    ...
    def parse_text_file(self, file_path: Path):
        ...
        """Parsea archivos de texto: cada línea después de [TEXT_SET:...] es una frase, o líneas directas para plantillas de libros"""
        ...
        try:
            ...
            # Check if this is a book template file (book_art.txt, book_instruction.txt)
            # These files don't have [TEXT_SET:...], they're just direct template lines
            is_book_template = "book_art" in file_path.name.lower() or "book_instruction" in file_path.name.lower()

            

            for i, line in enumerate(lines, 1):
                
                # For book template files, parse every non-empty line as a template
                if is_book_template:
                    if line.strip() and not line.strip().startswith('#'):
                        text = line.strip()
                        if text:
                            # Use filename as text_set for book templates
                            text_set = "BOOK_TEMPLATE"
                            if "book_art" in file_path.name.lower():
                                text_set = "BOOK_ART"
                            elif "book_instruction" in file_path.name.lower():
                                text_set = "BOOK_INSTRUCTION"
                            
                            entry = TextEntry(str(file_path.resolve()), i, text_set, text, text)
                            entry.entry_id = len(self.text_entries)
                            self.text_entries.append(entry)
                            self.text_entry_dict[entry.entry_id] = entry
                            entries_found += 1
    ...
    def save_text_file(self, file_path: str, entries: List[TextEntry]):
        ...
        for line_num, entry in changes.items():
            ...
            if line_num < len(lines):
                ...
                # Para archivos de plantillas de libros (book_art.txt, book_instruction.txt)
                # Simplemente escribir el texto directamente sin tags
                if entry.text_set in ["BOOK_ART", "BOOK_INSTRUCTION", "BOOK_TEMPLATE"]:
                    new_line = f"{clean_text}\n"
                    if new_line != original_line:
                        modifications_made += 1
                        self.log(f"    Line {line_num + 1}: {entry.text_set}")
                        self.log(f"      OLD: {original_line.strip()}")
                        self.log(f"      NEW: {new_line.strip()}")
                    lines[line_num] = new_line
                    continue
    ...
    def parse_json_response(self, response: str) -> list:
        ...
        """Parsea respuesta JSON de forma robusta y optimizada, manejando diferentes formatos"""
        ...
        # Usar patrón greedy que capture todo el array (incluyendo arrays anidados)
        ...
        for i, char in enumerate(response):
            if char == '[':
                ...
                if bracket_count == 0 and start_idx != -1:
                    # Encontramos un array balanceado
        ...
        # Fallback: usar regex si el método balanceado falla
    ...
    def call_deepseek_api(self, system_prompt: str, user_prompt: str, api_key: str = None, model: str = None) -> str:
        ...
        # Para respuestas JSON grandes, necesitamos más tokens
        # Estimación: ~50-100 tokens por entrada con frases largas
        max_tokens = min(16000, max(200, available_tokens * 2))  # Más tokens para respuestas JSON grandes
        ...
        try:
            ...
            response = requests.post(self.api_url, headers=headers, json=data, timeout=120)  # 120 second timeout to prevent hanging
            ...
            error_msg = f"API request timed out after 120 seconds: {e}"
    ...
    def get_cache_key(self, data: dict) -> str:
        ...
        cache_data = {
            "system": data.get("system", "")[:200],  # First 200 chars for key
            "user": data.get("user", "")[:200],
            "model": data.get("model", "")
        }
        json_str = json.dumps(cache_data, sort_keys=True, ensure_ascii=False)
        return hashlib.md5(json_str.encode('utf-8')).hexdigest()
    
    def call_deepseek_api_cached(self, system_prompt: str, user_prompt: str, api_key: str = None, model: str = None) -> str:
        """Cached API calls for identical requests"""
        if api_key is None:
            try:
                api_key = self.api_key_var.get() if hasattr(self, 'api_key_var') else self.api_key
            except:
        ...
        if cache_key in self.response_cache:
            self.log("Using cached API response")
            return self.response_cache[cache_key]
        
        result = self.call_deepseek_api(system_prompt, user_prompt, api_key, model)
        if result:
            self.response_cache[cache_key] = result
        return result
    
    def save_progress(self, generated_data: list, processed_count: int, total_entries: int):
        ...
        try:
            ...
            self.log(f"Progress saved: {processed_count} entries")
        except Exception as e:
    ...
    def cleanup_progress(self):
        ...
        try:
            ...
            self.log(f"Error cleaning up progress: {e}")
    
    def estimate_entry_size(self, entry) -> int:
        """Estimate token count for an entry (ULTRA-OPTIMIZED: we only send [index, type] now)"""
        if isinstance(entry, NameEntry):
            # Now we only send [index, type] - much smaller!
            # Index: ~5 tokens, type: ~3 tokens, JSON overhead: ~10 tokens
            return 18  # Fixed small size since we don't send parts anymore
        elif isinstance(entry, TextEntry):
            return len(entry.text) // 4 + 5
        else:
            return 20  # Default estimate
    
    def optimize_batch_sizes(self, entries: list, target_tokens: int = 1200) -> list:
        ...
        for entry in entries:
            ...
            if current_size + entry_size > target_tokens and current_batch:
    ...
    def process_chunk(self, chunk_entries: list, chunk_idx: int, prompts_list: list, 
                      rotate_every: int, api_key: str, model: str, start_idx: int) -> list:
        """Process one chunk of entries with parallel prompt assignment"""
        # Assign different prompts to different chunks for parallel processing
        # Each chunk gets a different prompt from the list, cycling through them
        if len(prompts_list) > 1:
            # Use chunk index to assign prompt (enables parallel calls with different prompts)
            prompt_idx = chunk_idx % len(prompts_list)
        else:
            prompt_idx = 0
        user_prompt = prompts_list[prompt_idx]
        
        # Log which prompt is being used for this chunk
        self.log(f"Chunk {chunk_idx + 1} using prompt {prompt_idx + 1}/{len(prompts_list)}: '{user_prompt[:50]}...'")
        
        # ULTRA-COMPACT: Only send index and type, NOT original parts (saves massive tokens!)
        entries_info = []
        for i, entry in enumerate(chunk_entries):
            idx = start_idx + i
            # Only [index, type] - AI doesn't need original parts to generate new names
            entries_info.append([idx, entry.tag_type])
        
        # Clear format instructions without specific examples
        system_prompt = f"""Generate Dwarf Fortress names following theme: "{user_prompt}"
...
Parts count: NAME/CASTE_NAME/NAME_*=3, NOUN/T_WORD=2, ADJ/STATE_NAME*=1, SQUAD=3(keep number), PLANT_NAME=3, VERB=5, CDI_VERB=2-3, COLOR_NAME/SHAPE_NAME/DESCRIPTOR_NAME=1-2, STATE_NAME=1"""
        
        # Ultra-compact JSON: only [index, type] pairs
...
Parts: NAME/CASTE_NAME/NAME_*=3, NOUN/T_WORD=2, ADJ/STATE_NAME*=1, SQUAD=3(keep number), PLANT_NAME=3, VERB=5, CDI_VERB=2-3, COLOR_NAME/SHAPE_NAME/DESCRIPTOR_NAME=1-2, STATE_NAME=1"""
        
        # Call API (with caching)
        result = self.call_deepseek_api_cached(system_prompt, user_prompt_full, api_key, model)
        
        if result:
            # Log response for debugging
            self.log(f"API Response length: {len(result)} characters")
            if len(result) > 500:
                self.log(f"Response preview (first 500): {result[:500]}")
                self.log(f"Response preview (last 500): {result[-500:]}")
            
            parsed = self.parse_json_response(result)
            if parsed:
                self.log(f"Successfully parsed {len(parsed)} entries from response")
                normalized_parsed = [self.normalize_json_entry(entry) for entry in parsed]
                return normalized_parsed
            else:
                self.log(f"WARNING: Failed to parse JSON from response")
                self.log(f"Trying to extract and fix JSON...")
                # Try to extract JSON manually and fix common issues
                import re
                ...
                if json_match:
                    ...
                    # Try to complete incomplete JSON by finding the last complete entry
                    try:
                        parsed = json.loads(json_str)
                        if isinstance(parsed, list):
                            self.log(f"Found JSON array with {len(parsed)} entries")
                            normalized_parsed = [self.normalize_json_entry(entry) for entry in parsed]
                            return normalized_parsed
                    except json.JSONDecodeError as e:
                        self.log(f"JSON parse error: {e}")
                        # Try to extract valid entries up to the error
                        try:
                            # Find the position of the error
                            error_pos = e.pos if hasattr(e, 'pos') else len(json_str)
                            # Try to find the last complete entry before the error
                            # Look for complete objects before the error position
                            partial_json = json_str[:error_pos]
                            # Find the last complete } before the error
                            last_complete = partial_json.rfind('}')
                            if last_complete > 0:
                                # Try to extract up to that point
                                try:
                                    # Find the array start
                                    array_start = partial_json.find('[')
                                    if array_start >= 0:
                                        # Try to complete the array
                                        truncated_json = partial_json[array_start:last_complete+1] + ']'
                                        parsed = json.loads(truncated_json)
                                        if isinstance(parsed, list) and len(parsed) > 0:
                                            self.log(f"Extracted {len(parsed)} valid entries from truncated JSON")
                                            normalized_parsed = [self.normalize_json_entry(entry) for entry in parsed]
                                            return normalized_parsed
                                except:
                                    pass
                        except Exception as e2:
                            self.log(f"Error trying to fix JSON: {e2}")
    ...
    def _generate_names_with_ai_thread(self, prompts_list: list, rotate_every: int, percentage: float):
        ...
        """Genera nombres en thread separado con procesamiento paralelo optimizado"""
        
        # Reset cancellation flag at start
        self.cancel_generation.clear()

        start_time = time.time()
        self.log(f"Starting optimized name generation with {percentage}% of entries")
        self.root.after(0, lambda: self.status_label.config(text="Generando nombres con AI (optimizado)..."))
        ...
        if progress:
            resume_msg = f"Found previous progress ({progress['processed_count']} entries). Resume?"
            # Use a simple flag for resume decision (auto-resume for now, can be made interactive)
            if progress.get('total_entries') == total_entries:
                all_generated = progress.get("generated", [])
                start_idx = progress.get("processed_count", 0)
                self.log(f"Resuming from checkpoint: {start_idx} entries already processed")
            else:
                all_generated = []
                start_idx = 0
                self.log("Previous progress doesn't match current data, starting fresh")
        else:
            all_generated = []
            start_idx = 0
        ...
        self.log(f"OPTIMIZED PROCESSING CONFIGURATION:")
        ...
        self.log(f"  Starting from index: {start_idx:,}")
        ...
        # Get remaining entries to process
        remaining_entries = self.name_entries[start_idx:start_idx + entries_to_process]
        if not remaining_entries:
            self.log("No entries to process")
            return
        
        # Optimize batch sizes (reduced to avoid JSON truncation)
        optimized_batches = self.optimize_batch_sizes(remaining_entries, target_tokens=1200)
        self.log(f"Created {len(optimized_batches)} optimized batches for parallel processing")
        
        # Process in parallel with ThreadPoolExecutor
        # Use more workers for faster processing - API calls are I/O bound, so we can have many concurrent requests
        # DeepSeek API doesn't have strict rate limits, but we'll use a reasonable number to balance speed and stability
        # For maximum speed: use up to 30 workers (or as many batches as we have, whichever is smaller)
        max_workers = min(30, len(optimized_batches))  # Up to 30 parallel API calls for maximum speed
        processed_count = len(all_generated)
        
        self.log(f"Starting parallel processing with {max_workers} workers (max 30 for speed)...")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for i, batch in enumerate(optimized_batches):
                ...
                batch_start_idx = batch_start_indices[i]
                future = executor.submit(
                    self.process_chunk,
                    batch, i, prompts_list, rotate_every, api_key, model, batch_start_idx
                )
                future_to_batch[future] = (i, batch_start_idx, len(batch))
            
            # Process results as they complete
            completed_batches = 0
            for future in as_completed(future_to_batch):
                # Check if cancelled before processing each result
                if self.cancel_generation.is_set():
                    self.log("Generation cancelled by user")
                ...
                try:
                    # Add timeout to result() call to prevent hanging (180 seconds max per batch)
                    batch_result = future.result(timeout=180)
                    if batch_result:
                        ...
                        rate = len(all_generated) / elapsed if elapsed > 0 else 0
                        remaining = entries_to_process - len(all_generated)
                        eta = remaining / rate if rate > 0 else 0
                        
                        self.log(f"Batch {batch_idx + 1}/{len(optimized_batches)} complete: "
                                f"{len(all_generated):,}/{entries_to_process:,} entries "
                                f"({progress_pct:.1f}%) - Rate: {rate:.1f} entries/sec - ETA: {eta/60:.1f} min")
                        
                        # Save progress every few batches
                        if completed_batches % 5 == 0 or completed_batches == len(optimized_batches):
                            # processed_count should be the absolute index, not relative
                            absolute_processed = start_idx + processed_count
                            self.save_progress(all_generated, absolute_processed, total_entries)
                    else:
                        self.log(f"WARNING: Batch {batch_idx + 1} returned no results")
                except TimeoutError:
                    self.log(f"ERROR: Batch {batch_idx + 1} timed out after 180 seconds - skipping")
                except Exception as e:
                    self.log(f"ERROR processing batch {batch_idx + 1}: {e}")
        
        # Clean up progress file on success (unless cancelled)
        if not self.cancel_generation.is_set():
            self.cleanup_progress()
        
        # Final performance summary
        total_time = time.time() - start_time
        final_rate = len(all_generated) / total_time if total_time > 0 else 0
        self.log(f"=" * 60)
        self.log(f"GENERATION COMPLETE:")
        self.log(f"  Total generated: {len(all_generated):,} entries")
        self.log(f"  Total time: {total_time/60:.1f} minutes ({total_time:.1f} seconds)")
        self.log(f"  Average rate: {final_rate:.1f} entries/second")
        self.log(f"=" * 60)
        ...
        def show_results():
            ...
            if all_generated:
                ...
                self.log(f"SUCCESS: Generated {len(all_generated)} names")
                messagebox.showinfo("Éxito", 
                    f"Generados {len(all_generated)} nombres ({percentage}% de {total_entries:,}).\n"
                    f"Tiempo: {total_time/60:.1f} minutos.\nRevisa y aplica.")
            else:
...
For each entry in input, generate a NEW text following the theme. Use "index","text","type" from input. Keep [PLACEHOLDERS] exact. Match count exactly."""
    ...
    def apply_ai_to_names(self):
        ...
        for gen_entry in generated:
            ...
            if entry.tag_type in ["NAME", "CASTE_NAME", "NAME_MALE", "NAME_FEMALE"]:
                # NAME, CASTE_NAME, NAME_MALE, NAME_FEMALE: formato singular:plural:adjective (3 partes máximo)
                max_parts = 3
            elif entry.tag_type.startswith("NAME_"):
                # NAME_WAR, NAME_BATTLE, etc. - Nombres de símbolos (1-3 partes)
                max_parts = 3
                ...
                # CDI_VERB: singular:plural:adjective (2-3 partes)
                max_parts = 3
            elif entry.tag_type in ["COLOR_NAME", "SHAPE_NAME", "DESCRIPTOR_NAME"]:
                # Descriptors: 1-2 partes (singular o singular:plural)
                max_parts = 2
            elif entry.tag_type == "STATE_NAME":
                # STATE_NAME: 1 parte (nombre del estado del material)
                max_parts = 1
            elif entry.tag_type in ["STATE_NAME_ADJ", "STATE_NAME"]:
